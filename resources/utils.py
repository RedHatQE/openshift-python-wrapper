...
def nudge_delete(name):
    ...
    try:
        ...
        LOGGER.info(f"Error happened while nudging namespace {name}: {exp}")
...
def retry_on_exceptions(func):
    def inner(*args, **kwargs):
        ...
        while timeout > 0:
            ...
            except openshift.dynamic.exceptions.InternalServerError as ex:
                if "etcdserver: leader changed" in ex:
                    exception = ex
                    LOGGER.warning("SSL ProtocolError")
                    time.sleep(sleep)
                    timeout = timeout - sleep
        ...
        if not res:
            raise urllib3.exceptions.ProtocolError(exception)
